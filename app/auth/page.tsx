...
export default function AuthPage() {
  ...
  async function callApi(path: string, body: any) {
    const res = await fetch(path, {
      ...
      credentials: "include", // ✅ clave para cookies (access_token)
    });
  ...
  async function ensureSession() {
    // ✅ confirma que la cookie quedó guardada y es válida
    const r = await fetch("/api/auth/me", { credentials: "include" });
    if (!r.ok) throw new Error("No se pudo validar la sesión. Intenta de nuevo.");
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const { res, json } = await callApi(endpoint, payload);

      if (!res.ok) {
        // tu fail(msg,status) probablemente devuelve { error: msg }
        throw new Error(json?.error || "Error en la operación");
      }

      if (mode === "login") {
        // ✅ opcional pero recomendado (evita casos donde cookie no se guardó por flags)
        await ensureSession();

        router.replace("/dashboard");
        router.refresh(); // ayuda si usas middleware/layout server
        return;
      }

      // registro ok -> vuelve a login y deja el email seteado
      setMode("login");
      ...
      <div className="w-full max-w-[420px] rounded-3xl bg-white dark:bg-slate-900 p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] border border-slate-100 dark:border-slate-800">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
            {mode === "login" ? "Ingresa tus credenciales para acceder" : "Completa el formulario para registrarte"}
          </p>
        </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={[
                  "w-full rounded-xl border border-slate-200 bg-slate-50 px-10 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:focus:border-slate-500 transition-all pr-10",
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-white dark:bg-slate-900 px-3 text-slate-400 dark:text-slate-500">o continuar con</span>
          </div>
        </div>
